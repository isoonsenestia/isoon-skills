...
2. **Profile and snap.** Run the `fe-next` repo profile, then snap each measured value: colour to a token, spacing to a scale step, type to a style, element to an existing component (reuse first).
...
6. **Handoff.** Build from both artifacts via `fe-next`, then verify with `verify-ui-against-design-headless` against the saved image, the checklist and the measured targets.
